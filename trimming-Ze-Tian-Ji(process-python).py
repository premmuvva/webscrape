...
link="https://m.wuxiaworld.co/Ze-Tian-Ji/"
...
string=[""]*len(soup)
incomplete=[]
file=open("Fighter of the Destiny.txt",'w')
...
def thread(a,b):
	try:
		...
		#	text=text+'\n\n\n\n\n\n\n'
		#	print(str(soup[link_number])+"end"+str(num))
...
#t1 = threading.Thread(target=thread(1,int(len(soup)/2)), name='t1') 
#t2 = threading.Thread(target=thread(int(len(soup)/2),len(soup)), name='t2') 
...
for chapter_number in range (1,len(soup)):
	if(string[chapter_number]!=''):
		print("incomplete "+str(chapter_number))
		continue
